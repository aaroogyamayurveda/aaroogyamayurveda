...
(function(){
  ...
  async function render(){
    ...
    async function load(){
      ...
      var rows={};
      function bucket(id){var k=id||'unassigned';if(!rows[k])rows[k]={name:agents.get(k)?.name||'Unassigned',code:agents.get(k)?.code||'',leads:0,worked:0,orders:0,delivered:0,cancelled:0,revenue:0};return rows[k]}
      (leadsR.data||[]).filter(function(x){return !String(x.lead_status||'').toLowerCase().includes('converted')||true}).forEach(function(x){bucket(x.assigned_to).leads++});
      (intR.data||[]).forEach(function(x){if(x.agent_id)bucket(x.agent_id).worked++});
      (ordersR.data||[]).filter(function(x){return !String(x.remarks||'').includes('[ENQUIRY]')}).forEach(function(x){var b=bucket(x.agent_id);b.orders++;b.revenue+=Number(x.total_amount||0);if(x.order_status==='delivered')b.delivered++;if(x.order_status==='cancelled'||x.order_status==='rto')b.cancelled++});
      var arr=Object.values(rows).sort(function(a,b){return b.orders-a.orders||b.leads-a.leads});
      var totalLeads=arr.reduce(function(a,x){return a+x.leads},0),totalWorked=arr.reduce(function(a,x){return a+x.worked},0),totalOrders=arr.reduce(function(a,x){return a+x.orders},0),totalDelivered=arr.reduce(function(a,x){return a+x.delivered},0),totalRevenue=arr.reduce(function(a,x){return a+x.revenue},0);
      body.innerHTML='<div class="cards"><div class="stat"><span>Leads</span><b>'+totalLeads+'</b></div><div class="stat"><span>Worked / Interactions</span><b>'+totalWorked+'</b></div><div class="stat"><span>Orders</span><b>'+totalOrders+'</b></div><div class="stat"><span>Conversion %</span><b>'+fmtPct(totalOrders,totalLeads)+'</b></div><div class="stat"><span>Delivered</span><b>'+totalDelivered+'</b></div><div class="stat"><span>Order Value</span><b>'+money(totalRevenue)+'</b></div></div><div class="tablewrap"><table><thead><tr><th>Rank</th><th>Agent</th><th>Leads</th><th>Worked</th><th>Orders</th><th>Conversion %</th><th>Delivered</th><th>Delivery %</th><th>Cancelled/RTO</th><th>Order Value</th></tr></thead><tbody>'+(arr.map(function(x,i){return '<tr><td>'+String(i+1)+'</td><td><b>'+esc(x.name)+'</b><div class="crm1-muted">'+esc(x.code)+'</div></td><td>'+x.leads+'</td><td>'+x.worked+'</td><td>'+x.orders+'</td><td>'+fmtPct(x.orders,x.leads)+'</td><td>'+x.delivered+'</td><td>'+fmtPct(x.delivered,x.orders)+'</td><td>'+x.cancelled+'</td><td>'+money(x.revenue)+'</td></tr>'}).join('')||'<tr><td colspan="10" class="empty">No agent activity for selected period</td></tr>')+'</tbody></table></div>';
    }
