...
function monthInputs(){
 ...
 if(v==='month'){
  if(from.type!=='month')from.type='month';
  if(to.type!=='month')to.type='month';
 }else if(v==='year'){
  if(from.type!=='number')from.type='number';
  if(to.type!=='number')to.type='number';
 }else{
  if(from.type!=='date')from.type='date';
  if(to.type!=='date')to.type='date';
 }
}
function queueNav(){
 ...
 if(q){q.classList.remove('crm1-role-v6-hide');q.style.removeProperty('display');q.style.removeProperty('visibility');return}
 var p=document.getElementById('crm1W2Queue');
 if(!p)return;
 var auth=window.sb&&window.sb.auth;
 if(!auth)return;
 auth.getUser().then(function(r){
  var u=r&&r.data&&r.data.user;if(!u)return;
  window.sb.from('profiles').select('role').eq('id',u.id).maybeSingle().then(function(x){
   ...
   b.onclick=function(){document.querySelectorAll('.page').forEach(function(el){el.classList.remove('active')});p.classList.add('active')};
   nav.appendChild(b);
  });
 }).catch(function(){});
}
