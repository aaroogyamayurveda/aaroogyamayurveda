...
function fmt(v){return v?new Intl.DateTimeFormat('en-IN',{timeZone:'Asia/Kolkata',dateStyle:'medium',timeStyle:'short'}).format(new Date(v)):'-'}
function addNav(){
 var nav=document.getElementById('nav');if(!nav||document.getElementById('crm1NavOrderTimeline'))return;
 var b=document.createElement('button');b.id='crm1NavOrderTimeline';b.type='button';b.textContent='🕒 Order Timeline';b.onclick=openPage;nav.appendChild(b);
}
function openPage(){
...
async function search(){
 ...
 try{
   ...
   var histories=hr.data||[],grouped={};histories.forEach(function(h){(grouped[h.order_id]||(grouped[h.order_id]=[])).push(h)});
 ...
 }catch(e){msg.textContent='Timeline error: '+(e.message||e)}
}
