/* CRM1 Settlements detailed reporting layer: dealer commission + courier freight/RTO accounting. */
(function(){
  'use strict';
  ...
  function setReady(v){var r=$('crm1SettDetailedRoot');if(r)r.dataset.crm1Ready=v?'1':'0'}
  function build(){
    ...
    c.innerHTML='<div id="crm1SettDetailedRoot">'+
      ...
      '<label>From <input id="crm1SetDFrom" type="date"></label>'+ 
      '<label>To <input id="crm1SetDTo" type="date"></label>'+ 
      ...
      '<div class="panel"><div class="tablewrap"><table><thead><tr><th>Rank</th><th>Partner</th><th>Type</th><th>Delivered</th><th>RTO</th><th>COD</th><th>Commission</th><th>Forward Freight</th><th>RTO Charges</th><th>Other</th><th>Net Settlement</th><th>Status</th><th>Period</th></tr></thead><tbody id="crm1SetDBody"></tbody></table></div></div>'+ 
      '<div class="panel"><h3>Generate Settlement</h3>'+ 
      '<div class="crm1-toolbar" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">'+
      '<input id="crm1SetDGenFrom" type="date"><input id="crm1SetDGenTo" type="date"><select id="crm1SetDPartner"><option value="">Select Partner</option></select>'+ 
      '<input id="crm1SetDCommission" type="number" min="0" step="0.01" placeholder="Dealer commission / delivered order">'+
      '<input id="crm1SetDForward" type="number" min="0" step="0.01" placeholder="Courier forward freight">'+
      '<input id="crm1SetDRto" type="number" min="0" step="0.01" placeholder="Courier RTO charges">'+
      '<input id="crm1SetDOther" type="number" step="0.01" placeholder="Other charges">'+
      '<input id="crm1SetDCod" type="number" min="0" step="0.01" placeholder="COD remitted (optional)">'+
      '<input id="crm1SetDAdj" type="number" step="0.01" placeholder="Adjustment (+/-)">'+
      '<button class="btn" id="crm1SetDGenerate">Generate</button></div>'+
      '<div id="crm1SetDHelp" class="crm1-muted" style="margin-top:8px">Dealer: Commission = Delivered × rate. Courier: no commission; use actual courier invoice/ledger freight, RTO and other charges. COD remitted defaults to delivered COD.</div></div>'+
      '</div>';
    var t=today();$('crm1SetDFrom').value=t;$('crm1SetDTo').value=t;$('crm1SetDGenFrom').value=t;$('crm1SetDGenTo').value=t;
    $('crm1SetDApply').onclick=load;
    $('crm1SetDToday').onclick=function(){$('crm1SetDFrom').value=today();$('crm1SetDTo').value=today();load()};
    $('crm1SetDGenerate').onclick=generate;
    $('crm1SetDPartner').onchange=updatePartnerFields;
  ...
  function updatePartnerFields(){
    var sel=$('crm1SetDPartner');if(!sel)return;
    var opt=sel.options[sel.selectedIndex];var type=opt?.dataset?.type||'';
    $('crm1SetDCommission').style.display=type==='dealer'?'inline-block':'none';
    $('crm1SetDForward').style.display=type==='courier'?'inline-block':'none';
    $('crm1SetDRto').style.display=type==='courier'?'inline-block':'none';
    $('crm1SetDOther').style.display=type==='courier'?'inline-block':'none';
    $('crm1SetDCod').style.display=type==='courier'?'inline-block':'none';
    $('crm1SetDAdj').style.display='inline-block';
    $('crm1SetDCommission').placeholder='Dealer commission / delivered order';
    if(type==='courier')$('crm1SetDHelp').textContent='Courier: commission is 0. Enter actual forward freight, RTO charges and other invoice/ledger charges. COD remitted can be entered manually; otherwise delivered COD is used.';
    else $('crm1SetDHelp').textContent='Dealer: Commission = Delivered × rate. If delivered = 0, commission = ₹0. Courier freight/RTO fields are not used for dealers.';
  }
  async function load(){
    ...
    try{
      ...
      $('crm1SetDKpis').innerHTML='<div class="stat"><span>Settlements</span><b>'+rows.length+'</b></div><div class="stat"><span>Delivered</span><b>'+delivered+'</b></div><div class="stat"><span>RTO</span><b>'+rto+'</b></div><div class="stat"><span>COD</span><b>'+money(cod)+'</b></div><div class="stat"><span>Commission</span><b>'+money(commission)+'</b></div><div class="stat"><span>Courier Charges</span><b>'+money(freight+rtoCharges+other)+'</b></div><div class="stat"><span>Net Settlement</span><b>'+money(net)+'</b></div>';
      rows.sort(function(a,b){return Number(b.net_payable||0)-Number(a.net_payable||0)});
      $('crm1SetDBody').innerHTML=rows.map(function(x,i){return '<tr><td><b>'+(i+1)+'</b></td><td><b>'+esc(x.profiles?.full_name||x.partner_id||'-')+'</b></td><td>'+esc(x.partner_type||'-')+'</td><td>'+Number(x.delivered_orders||0)+'</td><td>'+Number(x.rto_orders||0)+'</td><td>'+money(x.cod_amount)+'</td><td>'+money(x.commission_amount)+'</td><td>'+money(x.forward_freight)+'</td><td>'+money(x.rto_charges)+'</td><td>'+money(x.other_charges)+'</td><td><b>'+money(x.net_payable)+'</b></td><td><span class="pill">'+esc(x.status||'-')+'</span></td><td>'+esc(x.period_from)+' to '+esc(x.period_to)+'</td></tr>'}).join('')||'<tr><td colspan="13" class="empty">No settlements for selected period</td></tr>';
      msg.textContent='Report: '+from+' to '+to;
      setReady(true);
    }catch(e){msg.textContent='Report error: '+(e.message||e);$('crm1SetDBody').innerHTML='';setReady(true)}
  }
  async function generate(){
    var partner=$('crm1SetDPartner')?.value,from=$('crm1SetDGenFrom')?.value,to=$('crm1SetDGenTo')?.value;
    var commission=Number($('crm1SetDCommission')?.value||0),forward=Number($('crm1SetDForward')?.value||0),rto=Number($('crm1SetDRto')?.value||0),other=Number($('crm1SetDOther')?.value||0),cod=String($('crm1SetDCod')?.value||'').trim()===''?null:Number($('crm1SetDCod').value),adj=Number($('crm1SetDAdj')?.value||0);
    ...
    var opt=$('crm1SetDPartner').options[$('crm1SetDPartner').selectedIndex];var type=opt?.dataset?.type||'';
    if(type==='dealer' && commission<0)return alert('Dealer commission rate cannot be negative');
    if(type==='courier' && commission!==0){commission=0;$('crm1SetDCommission').value=''}
