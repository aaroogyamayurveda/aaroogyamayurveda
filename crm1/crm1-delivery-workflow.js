...
(function(){
  ...
  function hookActionHistory(){
    ...
    document.addEventListener('change',async function(e){var sel=e.target.closest('.statusSel,.courierStatusSel');if(!sel)return;var id=sel.dataset.order,oldStatus=sel.dataset.old||'';if(!id)return;var oldMap={};oldMap[id]={status:oldStatus||null};setTimeout(function(){scheduleReconcile([id],oldMap)},120)},true)
  }
  function enhanceStatusSelects(root,role){var statuses=ROLE_STATUS[role]||ROLE_STATUS.dealer;(root||document).querySelectorAll('.statusSel,.courierStatusSel').forEach(function(sel){if(sel.dataset.crm1DeliveryEnhanced==='1')return;sel.dataset.crm1DeliveryEnhanced='1';var value=sel.value;sel.innerHTML=statuses.map(function(s){return '<option value="'+esc(s)+'" '+(s===value?'selected':'')+'>'+esc(s.replace(/_/g,' '))+'</option>'}).join('');if(value&&statuses.indexOf(value)<0){var o=document.createElement('option');o.value=value;o.textContent=value.replace(/_/g,' ');o.selected=true;sel.appendChild(o)}})}
  ...
  function init(){if(started)return;started=true;hookActionHistory();watchPages();[500,1500,3000].forEach(function(ms){setTimeout(refreshActivePage,ms)})}
