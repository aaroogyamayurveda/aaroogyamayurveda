...
function bar(){
 ...
 const t=document.createElement('section');t.id='crm1T2TBar';t.className='panel';t.style.marginBottom='18px';t.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap"><div><h3 style="margin:0">☎ Telephony</h3><div class="sub">Dial through your system / Windows registered calling app</div></div><div class="pill">T2T</div></div><div class="grid3" style="margin-top:12px"><div class="field"><label>Customer Mobile</label><input id="crm1T2TNumber" inputmode="numeric" maxlength="10" placeholder="10 digit mobile"></div><div class="field" style="display:flex;align-items:end"><button class="btn" id="crm1T2TDial" type="button">Dial Call</button></div></div>';host.insertBefore(t,e.nextSibling);
 $('crm1T2TNumber').addEventListener('input',()=>{const n=digits($('crm1T2TNumber').value);$('crm1T2TNumber').value=n;syncManualNumberToOrder(n)});
 $('crm1T2TDial').onclick=()=>{const n=digits($('crm1T2TNumber').value||$('crm1DialNumber')?.value||$('pageMobile')?.value);if(!/^[6-9]\d{9}$/.test(n)){toast('Enter valid 10 digit mobile');return}syncManualNumberToOrder(n);window.location.href='tel:'+n};
