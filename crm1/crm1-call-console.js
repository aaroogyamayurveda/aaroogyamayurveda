...
function bar(){
 ...
 $('crm1DialNumber').addEventListener('change',()=>{const p=$('pageMobile');if(p&&digits(p.value)!==digits($('crm1DialNumber').value)){p.value=digits($('crm1DialNumber').value);p.dispatchEvent(new Event('input',{bubbles:true}))}});
...
async function startCall(){
 ...
 $('crm1DialNumber').value=mobile;$('crm1StartCall').disabled=true;$('crm1EndCall').disabled=false;$('ccStatus').textContent='Calling — use personal phone';
...
async function endCall(){
 ...
 active=null;$('crm1StartCall').disabled=false;$('crm1EndCall').disabled=true;$('ccStatus').textContent='Disposition required';
