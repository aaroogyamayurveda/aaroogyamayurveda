/* CRM1 Navigation UI v3: preserve every nav item, stable dashboard default, independent scrolling, grouped navigation, explicit sidebar collapse/expand and view modes. */
(function(){
  'use strict';
  ...
  function addStyles(){
    if($('crm1NavigationUIStyleV3'))return;
    var s=document.createElement('style');s.id='crm1NavigationUIStyleV3';
    s.textContent=`
      .layout.crm1-nav-layout{height:calc(100vh - 68px);min-height:0;overflow:hidden;grid-template-columns:235px minmax(0,1fr)}
      .side.crm1-nav-side{height:100%;min-height:0;overflow-y:auto;overflow-x:hidden;scrollbar-width:thin;position:relative;padding-top:14px;padding-bottom:22px;transition:width .18s ease,background .18s ease}
      ...
      .app.crm1-compact .side button{white-space:nowrap;text-align:center;padding:10px 6px}.app.crm1-compact .crm1-nav-icon{width:auto;margin:0}.app.crm1-compact .crm1-nav-label{display:none}
  ...
  function collectButtons(){
    ...
    return Array.prototype.filter.call(nav.querySelectorAll('button'),function(b){
      if(b.id==='crm1ViewBtn'||b.id==='crm1SidebarToggle'||b.closest('#crm1ViewMenu'))return false;
  ...
  function normalizeButton(b){
    ...
    var raw=String(b.textContent||'').trim();if(!raw)return;
    var first=Array.from(raw)[0]||'•',rest=raw.slice(first.length).trim();
  ...
  function rebuildGroups(){
    ...
    ordered.forEach(function(g){
      ...
      var head=document.createElement('div');head.className='crm1-nav-group-title';head.innerHTML='<span>'+g.title+'</span><span>'+(saved[g.id]?'›':'⌄')+'</span>';
      var body=document.createElement('div');body.className='crm1-nav-group-body';items.forEach(function(b){body.appendChild(b)});
  ...
  function ensureViewControl(){
    ...
    if(!$('crm1SidebarToggle')){
      var toggle=document.createElement('button');toggle.id='crm1SidebarToggle';toggle.className='crm1-sidebar-toggle';toggle.type='button';toggle.title='Collapse / Expand Sidebar';toggle.textContent='☰';
      host.insertBefore(toggle,host.querySelector('#crm1ViewBtn')||host.querySelector('#logout')||null);
      toggle.onclick=function(e){e.stopPropagation();setMode(mode()==='compact'?'sidebar':'compact')};
    }
    if($('crm1ViewBtn'))return;
    ...
    var menu=document.createElement('div');menu.id='crm1ViewMenu';menu.className='crm1-view-menu';menu.innerHTML='<button data-mode="sidebar">▤ Full Sidebar View</button><button data-mode="compact">▥ Compact / Collapsed Sidebar</button><button data-mode="topbar">☰ Top Bar View</button>';
  ...
  function buildTopbar(){
    ...
    nav.querySelectorAll('.crm1-nav-group').forEach(function(g){var tg=document.createElement('div');tg.className='crm1-topnav-group';var title=document.createElement('span');title.className='crm1-topnav-title';title.textContent=g.querySelector('.crm1-nav-group-title span')?.textContent||'';tg.appendChild(title);g.querySelectorAll('button').forEach(function(orig){var c=document.createElement('button');c.type='button';c.dataset.origId=orig.id;c.innerHTML=orig.innerHTML;c.className=orig.className;c.onclick=function(){userInteracted=true;orig.click()};tg.appendChild(c)});t.appendChild(tg)});
  ...
  function bindScrollAndNavigation(){
    ...
    document.addEventListener('click',function(e){var btn=e.target.closest&&e.target.closest('#nav button');if(!btn)return;userInteracted=true;saveSidebarScroll();var pos=s?s.scrollTop:0;requestAnimationFrame(function(){if(s)s.scrollTop=pos;resetMainScroll();syncTopbar()})},true);
    if(observer)return;observer=new MutationObserver(function(){scheduleRebuild()});var nav=$('nav');if(nav)observer.observe(nav,{childList:true,subtree:true});if(m)m.addEventListener('scroll',function(){},{passive:true});
  }

  function findDashboardButton(){var all=collectButtons();return all.find(function(b){return /dashboard/i.test(String(b.textContent||''))})||null}
  function forceDashboard(){if(userInteracted)return;var dash=$('dashboard');if(!dash)return;var active=document.querySelector('.main .page.active');if(active&&active.id==='dashboard')return;var b=findDashboardButton();if(b){b.click();resetMainScroll()}}

  function init(){
    if(started)return;started=true;addStyles();
    var layout=document.querySelector('.layout'),s=side(),m=main();if(layout)layout.classList.add('crm1-nav-layout');if(s)s.classList.add('crm1-nav-side');if(m)m.classList.add('crm1-nav-main');
