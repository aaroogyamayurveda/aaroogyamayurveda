/* CRM1 Delivery Partner Performance: single-source date-range reporting layer. */
(function(){
  'use strict';
  var started=false;
  ...
  function ensurePage(){
    var page=$('partnerPerformance');if(!page)return null;
    var title=page.querySelector('h2');if(title)title.textContent='Delivery Partners Performance';
    var sub=page.querySelector('.title .sub');if(sub)sub.textContent='Dealer and courier performance and SLA';
    var root=$('partnerPerformanceContent');if(!root)return null;
    if(!root.querySelector('#crm1PPRoot')){
      root.innerHTML='<div id="crm1PPRoot"><div class="crm1-toolbar"><label>From <input type="date" id="crm1PPFrom"></label><label>To <input type="date" id="crm1PPTo"></label><button class="btn" id="crm1PPApply">Apply</button><button class="btn alt" id="crm1PPToday">Today</button><span class="sub">Delivery partner performance by order date.</span></div><div id="crm1PPBody" class="crm1-muted">Loading...</div></div>';
      var t=today();$('crm1PPFrom').value=t;$('crm1PPTo').value=t;
      $('crm1PPApply').onclick=load;$('crm1PPToday').onclick=function(){var v=today();$('crm1PPFrom').value=v;$('crm1PPTo').value=v;load()};
    }
    return root;
  }
  async function load(){
    var root=ensurePage();if(!root||!window.sb)return;
    ...
    var er=or.error||dr.error||pr.error;
    if(er){$('crm1PPBody').innerHTML='<div class="msg">'+esc(er.message)+'</div>';return}
    ...
    (or.data||[]).filter(function(o){return !String(o.remarks||'').includes('[ENQUIRY]')}).forEach(function(o){
      var p=o.dealer_id?dealers.get(o.dealer_id):o.courier_manager_id?couriers.get(o.courier_manager_id):null;
      if(!p)return;
    ...
    $('crm1PPBody').innerHTML='<div class="cards"><div class="stat"><span>Partners</span><b>'+rows.length+'</b></div><div class="stat"><span>Assigned</span><b>'+ta+'</b></div><div class="stat"><span>Delivered</span><b>'+td+'</b></div><div class="stat"><span>Revenue</span><b>'+money(tr)+'</b></div></div><div class="tablewrap"><table><thead><tr><th>Rank</th><th>Delivery Partner</th><th>Type</th><th>Assigned</th><th>In Progress</th><th>Delivered</th><th>RTO</th><th>Cancelled</th><th>Delivery %</th><th>Order Value</th><th>Revenue</th></tr></thead><tbody>'+(rows.map(function(x,i){return '<tr><td><b>'+(i+1)+'</b></td><td><b>'+esc(x.name)+'</b></td><td>'+esc(x.type)+'</td><td>'+x.assigned+'</td><td>'+x.inprogress+'</td><td>'+x.delivered+'</td><td>'+x.rto+'</td><td>'+x.cancelled+'</td><td>'+pct(x.delivered,x.assigned)+'</td><td>'+money(x.value)+'</td><td><b>'+money(x.revenue)+'</b></td></tr>'}).join('')||'<tr><td colspan="11" class="empty">No delivery partner orders for selected period</td></tr>')+'</tbody></table></div>';
  }
  function render(){var page=$('partnerPerformance');if(!page||!page.classList.contains('active')||!window.sb)return;ensurePage();load().catch(function(e){var b=$('crm1PPBody');if(b)b.innerHTML='<div class="msg">'+esc(e.message||e)+'</div>'})}
  function start(){
    if(started)return;started=true;
    var ticks=0,t=setInterval(function(){var p=$('partnerPerformance');if(p&&p.classList.contains('active')){render();clearInterval(t)}if(++ticks>120)clearInterval(t)},500);
    document.addEventListener('click',function(e){if(e.target.closest('#nav button'))setTimeout(function(){var p=$('partnerPerformance');if(p&&p.classList.contains('active'))render()},250)});
  }
