 ...
 * Render/load lifecycle is guarded so delayed async responses cannot write into a page that
 * has already been replaced or navigated away from.
 */
(function(){
  'use strict';
  if(window.__crm1IstOpsFixLoaded)return;
  window.__crm1IstOpsFixLoaded=true;
  var started=false;
  var TZ='Asia/Kolkata';
  var renderSeq={partner:0,settlement:0};
  ...
  function partnerDomValid(token){return token===renderSeq.partner&&active('partnerPerformance')&&$('partnerPerformanceContent')&&$('crm1ISTPPFrom')&&$('crm1ISTPPTo')&&$('crm1ISTPPMsg')&&$('crm1ISTPPStats')&&$('crm1ISTPPBody')}
  function settlementDomValid(token){return token===renderSeq.settlement&&active('settlements')&&$('settlementsContent')&&$('crm1ISTSetFrom')&&$('crm1ISTSetTo')&&$('crm1ISTSetMsg')&&$('crm1ISTSetKpis')&&$('crm1ISTSetBody')&&$('crm1ISTPendingBody')}

  async function renderPartnerPerformance(){
    var p=$('partnerPerformance'),c=$('partnerPerformanceContent');
    if(!p||!c||!p.classList.contains('active')||!window.sb)return;
    var token=++renderSeq.partner;
    ...
    async function loadPartner(){
      if(!partnerDomValid(token))return;
      var from=$('crm1ISTPPFrom').value||istDate(),to=$('crm1ISTPPTo').value||from,rng=istRange(from,to),msg=$('crm1ISTPPMsg');
      if(!msg)return;
      ...
      try{
        ...
        var stats=$('crm1ISTPPStats'),body=$('crm1ISTPPBody'),m=$('crm1ISTPPMsg');
        if(!stats||!body||!m||!partnerDomValid(token))return;
        stats.innerHTML='<div class="stat"><span>Partners</span><b>'+arr.length+'</b></div><div class="stat"><span>Assigned</span><b>'+arr.reduce(function(a,x){return a+x.assigned},0)+'</b></div><div class="stat"><span>Delivered</span><b>'+arr.reduce(function(a,x){return a+x.delivered},0)+'</b></div><div class="stat"><span>Revenue</span><b>'+money(arr.reduce(function(a,x){return a+x.revenue},0))+'</b></div>';
        body.innerHTML=arr.map(function(x,i){return '<tr><td><b>'+(i+1)+'</b></td><td><b>'+esc(x.name)+'</b></td><td>'+esc(x.type)+'</td><td>'+x.assigned+'</td><td>'+x.inprogress+'</td><td>'+x.delivered+'</td><td>'+x.rto+'</td><td>'+x.cancelled+'</td><td>'+(x.assigned?(x.delivered/x.assigned*100).toFixed(1):'0.0')+'%</td><td>'+money(x.orderValue)+'</td><td><b>'+money(x.revenue)+'</b></td></tr>'}).join('')||'<tr><td colspan="11" class="empty">No delivery orders for selected period</td></tr>';
        m.textContent='Report: '+rng.fromDate+' to '+rng.toDate;
      }catch(e){
        if(!partnerDomValid(token))return;
        var em=$('crm1ISTPPMsg'),eb=$('crm1ISTPPBody');
        if(em)em.textContent='Report error: '+(e.message||e);if(eb)eb.innerHTML='';
      }
    }
  }

  async function renderSettlements(){
    var p=$('settlements'),c=$('settlementsContent');
    if(!p||!c||!p.classList.contains('active')||!window.sb)return;
    var token=++renderSeq.settlement;
    ...
    async function loadSettlement(){
      if(!settlementDomValid(token))return;
      var from=$('crm1ISTSetFrom').value||istDate(),to=$('crm1ISTSetTo').value||from,rng=istRange(from,to),msg=$('crm1ISTSetMsg');if(!msg)return;msg.textContent='Loading…';
      try{
        var maps=await partnerMaps(),dealers=maps[0].data||[],couriers=maps[1].data||[];if(maps[0].error)throw maps[0].error;if(maps[1].error)throw maps[1].error;if(!settlementDomValid(token))return;var dm=new Map(dealers.map(function(x){return[x.id,x.dealer_name]})),cm=new Map(couriers.map(function(x){return[x.id,x.full_name]}));
        var s=await window.sb.from('partner_settlements').select('id,partner_id,period_from,period_to,delivered_orders,cod_amount,commission_amount,net_payable,status,partner_type,created_at').gte('period_from',rng.fromDate).lte('period_to',rng.toDate).order('period_from',{ascending:false});if(s.error)throw s.error;var rows=s.data||[];if(!settlementDomValid(token))return;
        ...
        var o=await window.sb.from('orders').select('id,order_no,order_status,total_amount,order_date,dealer_id,courier_manager_id,customer_id,customers(customer_name),remarks').eq('order_status','delivered').gte('order_date',rng.from).lt('order_date',rng.to).or('remarks.is.null,remarks.not.ilike.%[ENQUIRY]%');if(o.error)throw o.error;var orders=o.data||[];if(!settlementDomValid(token))return;var existing=new Set();rows.forEach(function(x){existing.add(String(x.partner_id)+'|'+x.period_from+'|'+x.period_to)});
