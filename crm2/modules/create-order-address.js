...
const setOptions=(select,items,placeholder,selected='',disabledWhenEmpty=true)=>{if(!select)return;select.innerHTML=`<option value="">${placeholder}</option>`+items.map(x=>`<option value="${esc(x)}" ${x===selected?'selected':''}>${esc(x)}</option>`).join('');select.disabled=disabledWhenEmpty&&!items.length};
...
const wire=()=>{
  ...
  pin?.addEventListener('input',async e=>{e.target.value=e.target.value.replace(/\D/g,'').slice(0,6);if(e.target.value.length!==6){rows=[];setOptions(state,[],'Select State','',false);setOptions(city,[],'Select City','',true);setOptions(post,[],'Select Area / Post','',true);return}if(msg)msg.textContent='Searching pincode…';rows=await fetchPincode(e.target.value);if(!rows.length){if(msg)msg.textContent='Pincode lookup unavailable. Enter State/City/Area manually.';return}const states=[...new Set(rows.map(x=>x.state))].sort();setOptions(state,states,'Select State',states[0]);refreshCities();if(city.options.length===2)city.value=rows[0].city;refreshPosts();if(post.options.length===2)post.value=rows[0].post;if(msg)msg.textContent='Pincode matched.';});
