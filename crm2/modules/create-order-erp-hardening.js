...
const error=(id,msg)=>{const el=$(id);if(!el)return;const next=msg||'';if(el.textContent!==next)el.textContent=next;const hidden=!msg;if(el.hidden!==hidden)el.hidden=hidden;el.classList.add('crm2-validation-error')};
...
function showLoginError(msg){let el=$('crm2LoginValidation');if(!el){el=document.createElement('div');el.id='crm2LoginValidation';el.className='field-error crm2-validation-error';$('loginForm')?.appendChild(el)}if(el.textContent!==msg)el.textContent=msg;if(el.hidden!==false)el.hidden=false}
function clearLoginError(){const el=$('crm2LoginValidation');if(el){if(el.textContent!=='')el.textContent='';if(el.hidden!==true)el.hidden=true}}
function setFieldError(id,msg){const field=$(id)?.closest('.field');if(!field)return;let el=field.querySelector('.crm2-inline-error');if(!el){el=document.createElement('div');el.className='crm2-inline-error field-error crm2-validation-error';field.appendChild(el)}if(el.textContent!==msg)el.textContent=msg;if(el.hidden!==!msg)el.hidden=!msg}
...
function hardenCallConsole(){
  ...
  const sync=()=>{const realStart=$('crm2CallStart'),realEnd=$('crm2CallEnd'),realTimer=$('crm2CallTimer');if(realStart&&start.disabled!==realStart.disabled)start.disabled=realStart.disabled;if(realEnd&&end.disabled!==realEnd.disabled)end.disabled=realEnd.disabled;const nextTimer=realTimer?.textContent||'00:00';if(timer.textContent!==nextTimer)timer.textContent=nextTimer};
...
function sanitizeVisibleHindi(){const body=document.body;if(!body)return;const bad=/कृपया|चयन करें|भरें|सही|करें|पहले valid|number enter|select karein|required hai|invalid hai/i;body.querySelectorAll('.field-error,.msg,.error').forEach(el=>{if(bad.test(el.textContent||'')){const next='Please check the highlighted field and enter valid information.';if(el.textContent!==next){el.textContent=next;el.classList.add('crm2-validation-error')}}})}
