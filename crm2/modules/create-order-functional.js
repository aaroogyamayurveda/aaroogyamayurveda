...
function renderCall(){
  ...
  if(activeCall){const sec=(Date.now()-new Date(activeCall.startedAt).getTime())/1000;if(timer)timer.textContent=formatTimer(sec);if($('crm1ParityCallTimer'))$('crm1ParityCallTimer').value=formatTimer(sec);if(start)start.disabled=true;if(end)end.disabled=false;if(parityStart)parityStart.disabled=true;if(parityEnd)parityEnd.disabled=false;setCallStatus('Calling / In Call')}
  else{if(start)start.disabled=false;if(end)end.disabled=true;if(parityStart)parityStart.disabled=false;if(parityEnd)parityEnd.disabled=true;if(timer&&timer.textContent==='')timer.textContent='00:00'}
}
...
async function startCall(){
  ...
  if(!row?.call_id){setCallStatus('Unable to start call: no call record returned.');return}
  activeCall={id:row.call_id,leadId:row.lead_id||null,customerId:row.customer_id||null,agentId:user,startedAt:row.started_at||new Date().toISOString()};persistActive();
...
async function endCall(){
  ...
  const outcome=text($('crm2CallOutcome')?.value)||null;const notes=text($('crm2CallNotes')?.value)||null;
  ...
  const duration=Number((Array.isArray(data)?data[0]:data)?.duration_seconds||0);if($('crm2CallTimer'))$('crm2CallTimer').textContent=formatTimer(duration);if($('crm1ParityCallTimer'))$('crm1ParityCallTimer').value=formatTimer(duration);renderCall();setCallStatus('Call completed');
}

async function logManualCall(){
  const m=mobile();if(!validMobile(m)){setCallStatus('Enter a valid 10-digit mobile first.');return}
  const outcome=text($('crm2CallOutcome')?.value)||'Manual Call Logged';const notes=text($('crm2CallNotes')?.value)||null;const duration=Math.max(0,Number($('crm2CallDuration')?.value)||0);
  setCallStatus('Logging manual call…');const {error}=await sb.rpc('crm2_log_manual_call',{p_mobile:m,p_outcome:outcome,p_notes:notes,p_duration_seconds:duration});
  if(error){setCallStatus('Unable to log manual call: '+(error.message||'database error'));return}setCallStatus('Manual call logged successfully');
}
...
async function loadDisposition(){
  ...
  const rows=data||[],parents=rows.filter(x=>x.level_no===1||x.parent_id===null),display={Lead:'Lead Case','Non Lead':'Non-Lead',Sales:'Sales Order',Language:'Language','Not Connected':'Not Connected',Transfer:'Transfer'};
  ...
  l1.onchange=updateChildren;l2.onchange=()=>{const cb=$('crm2FunctionalCallback');if(cb)cb.hidden=l2.options[l2.selectedIndex]?.textContent?.trim()!=='Call Back'};
  callbackBox(l1.closest('.panel')||l1.parentElement?.parentElement);
}
...
async function loadStatus(){
  ...
  const {data}=await sb.rpc('crm2_get_agent_status');const row=Array.isArray(data)?data[0]:data;if(row?.status)select.value=row.status;
  if(!select.dataset.functionalStatusBound){select.dataset.functionalStatusBound='1';select.addEventListener('change',async()=>{const value=select.value;select.disabled=true;const ok=await setStatus(value);select.disabled=false;if(ok)setCallStatus(`Agent status: ${value}`)})}
}

function collectPayload(){
  const l1=$('crm1Disposition1'),l2=$('crm1Disposition2'),product=$('crm2OrderProduct')?.selectedOptions?.[0];
  const quantity=Math.max(1,Number($('crm2OrderQty')?.value)||1),price=Math.max(0,Number(product?.dataset?.price||$('crm2OrderPrice')?.value||0));
  return {mobile:mobile(),name:text($('crm2OrderName')?.value),alternate_mobile:normalizeMobile($('crm2OrderAlt')?.value||''),age:text($('crm2OrderAge')?.value),gender:text($('crm2OrderGender')?.value),address:text($('crm2OrderAddress')?.value),post:text($('crm2OrderPost')?.value),city:text($('crm2OrderCity')?.value),state:text($('crm2OrderState')?.value),pincode:text($('crm2OrderPincode')?.value),product_id:product?.value||'',product_name:product?.dataset?.name||'',sku:product?.dataset?.sku||'',quantity,unit_price:price,payment_mode:$('crm2OrderPayment')?.value||'COD',source:$('crm2OrderSource')?.value||'Manual',campaign_id:$('crm2OrderCampaign')?.value||'',remarks:text($('crm2OrderRemarks')?.value),disposition_l1_id:l1?.value||'',disposition_l2_id:l2?.value||'',call_id:activeCall?.id||null,duration_seconds:activeCall?Math.round((Date.now()-new Date(activeCall.startedAt).getTime())/1000):Number($('crm2CallDuration')?.value||0),callback_at:$('crm2FunctionalCallbackAt')?.value||'',callback_priority:$('crm2FunctionalCallbackPriority')?.value||'normal'};
}

async function submitDisposition(){
  const l1=$('crm1Disposition1'),l2=$('crm1Disposition2');if(!l1?.value){setMessage('Please select Disposition Level 1.');return}if(!l2?.value){setMessage('Please select Disposition Level 2.');return}
  const payload=collectPayload();if(!validMobile(payload.mobile)){setMessage('Valid customer mobile required.');return}
  ...
  if(isSales){if(!payload.name){setMessage('Customer name required.');return}if(!payload.address||payload.address.length<5||!/^\d{6}$/.test(payload.pincode)||!payload.state||!payload.city||!payload.post){setMessage('Complete delivery address including Area/Post required.');return}if(!payload.product_id){setMessage('Please select a product.');return}if(payload.unit_price<=0){setMessage('Valid product price required.');return}}
  const button=$('crm1SubmitDisposition');if(button){button.disabled=true;button.textContent='Saving…'}setMessage('Saving disposition…');
  ...
  const result=Array.isArray(data)?data[0]:data;if(result?.type==='order')setMessage(`Order ${result.order_code||'created'} successfully.`,true);else if(result?.type==='callback')setMessage('Callback scheduled successfully.',true);else setMessage(`Disposition saved: ${result?.level_2||l2Name}.`,true);
  if(activeCall){activeCall=null;persistActive();stopTimer();renderCall()}if(button){button.disabled=false;button.textContent='Submit Disposition'}window.dispatchEvent(new CustomEvent('crm2DispositionSaved',{detail:result}));
}

function wireButtons(){
  if($('crm2CallStart'))$('crm2CallStart').onclick=startCall;if($('crm2CallEnd'))$('crm2CallEnd').onclick=endCall;if($('crm2ParityStartCall'))$('crm2ParityStartCall').onclick=startCall;if($('crm2ParityEndCall'))$('crm2ParityEndCall').onclick=endCall;if($('crm2ParityLogManual'))$('crm2ParityLogManual').onclick=logManualCall;if($('crm2ParityCallMobile'))$('crm2ParityCallMobile').onclick=e=>{e.preventDefault();startCall()};if($('crm1SubmitDisposition'))$('crm1SubmitDisposition').onclick=submitDisposition;loadStatus();loadDisposition()
}
async function restore(){try{const saved=JSON.parse(localStorage.getItem(ACTIVE_KEY)||'null');if(!saved?.id||!saved?.startedAt)return;const user=(await currentProfile())?.id||null;if(!user){localStorage.removeItem(ACTIVE_KEY);return}activeCall={...saved,agentId:saved.agentId||user};startTimer()}catch(e){console.warn('CRM2 active call restore failed',e)}}
function wire(){const ws=document.querySelector(WS);if(!ws)return;if(wiredWorkspace!==ws){wiredWorkspace=ws;wireButtons();restore()}else{if($('crm1SubmitDisposition'))$('crm1SubmitDisposition').onclick=submitDisposition;if($('crm2ParityStartCall'))$('crm2ParityStartCall').onclick=startCall;if($('crm2ParityEndCall'))$('crm2ParityEndCall').onclick=endCall;if($('crm2ParityLogManual'))$('crm2ParityLogManual').onclick=logManualCall}renderCall()}
const observer=new MutationObserver(wire);if(document.body)observer.observe(document.body,{subtree:true,childList:true});window.addEventListener('beforeunload',persistActive);wire();
