...
window.addEventListener('crm2CreateOrderParityReady',()=>{wireButtons();forceRender()});window.addEventListener('crm2FunctionalStartCall',startCall);window.addEventListener('crm2FunctionalEndCall',endCall);window.addEventListener('crm2FunctionalLogManualCall',logManualCall);window.addEventListener('crm2FunctionalCallRequest',startCall);window.addEventListener('beforeunload',persistActive);const observer=new MutationObserver(()=>{const ws=document.querySelector(WS);if(ws!==wiredWorkspace)wire()});if(document.body)observer.observe(document.body,{subtree:true,childList:true});wire();
