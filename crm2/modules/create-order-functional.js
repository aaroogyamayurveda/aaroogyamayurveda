...
async function startCall(){
  ...
  const row=Array.isArray(data)?data[0]:data;if(!row?.call_id){setCallStatus('Unable to start call: no call record returned.');return}
  ...
  window.crm2CreateOrderContext={...(window.crm2CreateOrderContext||{}),lead:{...(window.crm2CreateOrderContext?.lead||{}),id:activeCall.leadId}};startTimer();
}
...
async function endCall(){
  ...
  lastCallId=call.id;activeCall=null;persistActive();const duration=Number((Array.isArray(data)?data[0]:data)?.duration_seconds||0);if($('crm2CallTimer'))$('crm2CallTimer').textContent=formatTimer(duration);if($('crm1ParityCallTimer'))$('crm1ParityCallTimer').value=formatTimer(duration);renderCall();setCallStatus('Call completed');
}
