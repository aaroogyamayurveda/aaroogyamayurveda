...
window.addEventListener('crm2CreateOrderParityReady',forceRender);window.addEventListener('crm2FunctionalStartCall',startCall);window.addEventListener('crm2FunctionalEndCall',endCall);window.addEventListener('crm2FunctionalLogManualCall',logManualCall);window.addEventListener('crm2FunctionalCallRequest',startCall);window.addEventListener('beforeunload',persistActive);const observer=new MutationObserver(wire);if(document.body)observer.observe(document.body,{subtree:true,childList:true});wire();
