...
export async function createOrderFromLead({lead,productId,quantity,unitPrice,paymentMode,priority,remarks}){
  ...
  const {data:order,error}=await sb.from('orders').insert({customer_id:customerId,lead_id:lead.id,product_id:productId||null,agent_id:(await sb.auth.getUser()).data.user?.id||null,total,discount:0,payment_mode:paymentMode||'COD',priority:priority||'normal',remarks:remarks||null,status:'confirmed'}).select().single();
...
export async function changeShipmentStatus(shipmentId,status,remarks=''){
  ...
  const {error:updateError}=await sb.from('shipments').update({status,shipped_at:status==='dispatched'?new Date().toISOString():undefined,delivered_at:status==='delivered'?new Date().toISOString():undefined}).eq('id',shipmentId);if(updateError)throw updateError;
...
export async function createNdrCase({shipmentId,reason,remarks,nextActionAt}){
  ...
  const {data,error}=await sb.from('ndr_cases').insert({shipment_id:shipmentId,reason:reason||null,attempt_no:1,next_action_at:nextActionAt||null,status:'open'}).select().single();if(error)throw error;
  return data;
}
...
export async function createRtoCase({shipmentId,reason,remarks}){
  ...
  const {data,error}=await sb.from('rto_cases').insert({shipment_id:shipmentId,reason:reason||null,status:'open',inspection_status:remarks||null,restocked:false}).select().single();if(error)throw error;
  return data;
}

export async function recordPayment({orderId,amount,mode,status='collected',reference,type='collection'}){
  const {data,error}=await sb.from('payments').insert({order_id:orderId,amount:Math.max(0,Number(amount)||0),mode:mode||'COD',status,type,reference:reference||null}).select().single();if(error)throw error;return data;
}
