...
test.describe('CRM2 role access', () => {
  ...
  for (const [role, emailKey, passwordKey] of roles) {
    test(`${role}: login, role identity and logout`, async ({ page }) => {
      ...
      await expect(page.getByText(new RegExp(`\\b${role}\\b`, 'i'))).toBeVisible();
  ...
  test('super_admin: management nav opens quickly and marks the selected workspace active', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Lead Assignment', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Lead Assignment', exact: true })).toBeVisible({ timeout: 8000 });
    await expect(page.getByRole('button', { name: 'Lead Assignment', exact: true })).toHaveClass(/active/);
    await page.getByRole('button', { name: 'MIS Drilldown', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'MIS Drilldown', exact: true })).toBeVisible({ timeout: 8000 });
  ...
  test('specialist roles: navigation matches operational scope', async ({ page }) => {
    ...
    for (const [emailKey, passwordKey, visible, hidden] of cases) {
      const c = credentials(emailKey, passwordKey);
      test.skip(!c.email || !c.password, `Missing secrets for ${emailKey}`);
      await login(page, c.email, c.password);
      for (const label of visible) await expect(page.getByRole('button', { name: label, exact: true })).toBeVisible();
      for (const label of hidden) await expect(page.getByRole('button', { name: label, exact: true })).toBeHidden();
      await page.getByRole('button', { name: 'Logout', exact: true }).click();
    }
  });
  ...
  test('agent: management-only navigation is hidden', async ({ page }) => {
    ...
    await login(page, c.email, c.password);
    for (const label of ['Admin / Config', 'Lead Assignment', 'MIS Drilldown']) {
