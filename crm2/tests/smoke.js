...
const index=read('index.html'),app=read('app.js'),data=read('data.js'),workflow=read('modules/workflows.js'),config=read('config.js');
...
ok('no service role secret',!config.includes('service_role')&&!app.includes('service_role')&&!data.includes('service_role')&&!workflow.includes('service_role'));
ok('no CRM1 navigation dependency',!app.includes('../crm/')&&!app.includes('../crm1/'));
ok('no Google Drive dependency',!app.includes('Google Drive')&&!data.includes('Google Drive')&&!workflow.includes('Google Drive'));
