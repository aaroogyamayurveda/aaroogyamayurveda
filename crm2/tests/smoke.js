...
const orderRulesMigration=path.join(repoRoot,'supabase','migrations','20260911100000_crm2_create_order_duplicate_and_lock_rules.sql');
const orderSql=fs.existsSync(orderMigration)?fs.readFileSync(orderMigration,'utf8').toLowerCase():'';
const orderRulesSql=fs.existsSync(orderRulesMigration)?fs.readFileSync(orderRulesMigration,'utf8').toLowerCase():'';
...
ok('index loads create order calling module',/src="\.\/modules\/create-order-calling\.js\?v=\d+-\d+/.test(index));
...
ok('no service role secret',![config,app,data,workflow,finance,ops,admin,management,imports,createOrder,createOrderBridge,createOrderAddress,createOrderCalling].some(x=>x.includes('service_role')));
ok('no CRM1 navigation dependency',![app,workflow,finance,ops,admin,management,imports,createOrder,createOrderBridge,createOrderAddress,createOrderCalling].some(x=>x.includes('../crm/'))&&! [app,workflow,finance,ops,admin,management,imports,createOrder,createOrderBridge,createOrderAddress,createOrderCalling].some(x=>x.includes('../crm1/')));
ok('no Google Drive dependency',![app,data,workflow,finance,ops,admin,management,imports,createOrder,createOrderBridge,createOrderAddress,createOrderCalling].some(x=>x.includes('Google Drive')));
...
ok('Fast Order is disabled and not intercepted',createOrderBridge.includes('crm2FastOrderDisabled')&&!createOrderBridge.includes('stopImmediatePropagation')&&!createOrderBridge.includes("addEventListener('click'"));
ok('create order call lifecycle has Start/End controls and timer',createOrderCalling.includes("start.id='crm2CallStart'")&&createOrderCalling.includes("end.id='crm2CallEnd'")&&createOrderCalling.includes("timer.id='crm2CallTimer'")&&createOrderCalling.includes('duration_seconds'));
...
ok('Create Order uses Order Number as business identifier',createOrder.includes('order_code')||createOrder.includes('Order Number')||orderRulesSql.includes('order_code'));
ok('Create Order blocks duplicate active mobile + scheme + price orders',orderRulesSql.includes('pg_advisory_xact_lock')&&orderRulesSql.includes('duplicate_active_order')&&orderRulesSql.includes("status not in ('delivered','rto','cancelled')"));
ok('Delivered/RTO/Cancelled are terminal locked statuses',orderRulesSql.includes("status in ('delivered','rto','cancelled')")&&orderRulesSql.includes('permanently locked'));
ok('closed orders cannot be edited at database layer',orderRulesSql.includes('create trigger crm2_closed_order_lock')&&orderRulesSql.includes('create trigger crm2_closed_order_item_lock')&&orderRulesSql.includes('closed_order_locked'));
ok('Dealer/Courier delivery quantity adjustment is audited',orderRulesSql.includes('delivery_quantity_updated')&&orderRulesSql.includes("'dealer_manager','dispatch'")&&orderRulesSql.includes('delivery quantity'));
