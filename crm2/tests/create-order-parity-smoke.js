...
const index=read('index.html'),parity=read('modules/create-order-crm1-parity.js'),orderUi=read('modules/create-order-ui.js'),css=read('create-order-crm1-parity.css'),bridge=read('modules/create-order-bridge.js');
...
ok('manual call controls match CRM1 labels',parity.includes('Start Call')&&parity.includes('End Call')&&parity.includes('Log Manual Call'));
...
ok('CRM1 validation rules are enforced',parity.includes('Mobile exactly 10 digits')&&parity.includes('Pincode exactly 6 digits')&&parity.includes('Number.isInteger(age)')&&parity.includes('age > 120')&&parity.includes('qty > 99'));
ok('price and discount are frozen',parity.includes('price.readOnly = true')&&parity.includes("discount.value = '0'")&&parity.includes('discount.disabled = true'));
